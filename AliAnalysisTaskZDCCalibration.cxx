...
AliAnalysisTaskZDCCalibration::AliAnalysisTaskZDCCalibration() : AliAnalysisTaskSE(), 
    ...
    fProfileTPCPosNegCorr(nullptr),

    fHist2DTPCPsiCentTot(nullptr),
    fHist2DTPCPsiPosCentTot(nullptr),
    fHist2DTPCPsiNegCentTot(nullptr)
{
    for (int i = 0; i < 2; i++) {fHistPt[i] = nullptr;}
    for (int i = 0; i < 2; i++) {fHistEta[i] = nullptr;}
    for (int i = 0; i < 2; i++) {fHistPhi[i] = nullptr;}
    
    for (int i = 0; i < 3; i++) {fVtx[i] = -999;}
    for (int i = 0; i < 2; i++) {fHist2DCentCorr[i] = nullptr;}
    for (int i = 0; i < 2; i++)
    {
    ...
    for (int iRun = 0; iRun < fnRunMax; iRun++)
    {
      ...
      fHsit2DTPCPsiCent[iRun] = nullptr;
      fHist2DTPCPsiPosCent[iRun] = nullptr;
      fHist2DTPCPsiNegCent[iRun] = nullptr;
    }
...
void AliAnalysisTaskZDCCalibration::UserCreateOutputObjects()
{
    ...
    if (bCalculateV2)
    {
      ...
      for (int i = 0; i < 3; i++)
      {
        ...
        fProfileTPCPosNegCorr = new TProfile("fProfileTPCPosNegCorr", "fProfileTPCPosNegCorr", 10, 0., 100.);
        fOutputList->Add(fProfileTPCPosNegCorr);
    ...
    for (int iRun = 0; iRun < fAllRunsNum; iRun++)
    {
      ...
      if (bFillQAHist)
      {
        ...
        if (fDataSet.Contains("10h")) 
        {
          ...
          fHsit2DTPCPsiCent[iRun] = new TH2D("fHist2DTPCPsiCent",Form("Hist2D Cent vs. TPCPsi Run%d",runNumList10h[iRun]),10,0.,100.,50,0.,TMath::TwoPi());
          fHist2DTPCPsiPosCent[iRun] = new TH2D("fHist2DTPCPsiPosCent",Form("Hist2D Cent vs. TPCPsiPos Run%d",runNumList10h[iRun]),10,0.,100.,50,0.,TMath::TwoPi());
          fHist2DTPCPsiNegCent[iRun] = new TH2D("fHist2DTPCPsiNegCent",Form("Hist2D Cent vs. TPCPsiNeg Run%d",runNumList10h[iRun]),10,0.,100.,50,0.,TMath::TwoPi());
        }
        ...
        if (fDataSet.Contains("11h")) 
        {
          ...
          fHsit2DTPCPsiCent[iRun] = new TH2D("fHist2DTPCPsiCent",Form("Hist2D Cent vs. TPCPsi Run%d",runNumList11h[iRun]),10,0.,100.,50,0.,TMath::TwoPi());
          fHist2DTPCPsiPosCent[iRun] = new TH2D("fHist2DTPCPsiPosCent",Form("Hist2D Cent vs. TPCPsiPos Run%d",runNumList11h[iRun]),10,0.,100.,50,0.,TMath::TwoPi());
          fHist2DTPCPsiNegCent[iRun] = new TH2D("fHist2DTPCPsiNegCent",Form("Hist2D Cent vs. TPCPsiNeg Run%d",runNumList11h[iRun]),10,0.,100.,50,0.,TMath::TwoPi());
        }
        ...
        fQAListThisRun[iRun]->Add(fHsit2DTPCPsiCent[iRun]);
        fQAListThisRun[iRun]->Add(fHist2DTPCPsiPosCent[iRun]);
        fQAListThisRun[iRun]->Add(fHist2DTPCPsiNegCent[iRun]);
...
void AliAnalysisTaskZDCCalibration::UserExec(Option_t *)
{
    ...
    if (bCalculateV2) {
      ...
      for(Int_t iTrack(0); iTrack < nTracks; iTrack++) {
        ...
        fHist2DPhiPsiZNCVsCentPt[0] -> Fill(centV0M, pt, phi-PsiZNAGE);
        fHist2DPhiPsiZNAVsCentPt[0] -> Fill(centV0M, pt, phi-PsiZNAGE);
        fHist2DPhiPsiZNCVsCentPt[1] -> Fill(centV0M, pt, phi-PsiZNARC);
        fHist2DPhiPsiZNAVsCentPt[1] -> Fill(centV0M, pt, phi-PsiZNARC);
        fHist2DPhiPsiZNCVsCentPt[2] -> Fill(centV0M, pt, phi-PsiZNASF);
        fHist2DPhiPsiZNCVsCentPt[2] -> Fill(centV0M, pt, phi-PsiZNASF);
      }
      ...
      fHist2DTPCPsiCentTot    -> Fill(fCentrality, PsiTPC);
      fHist2DTPCPsiPosCentTot -> Fill(fCentrality, PsiTPCPos);
      fHist2DTPCPsiNegCentTot -> Fill(fCentrality, PsiTPCNeg);
      
      fHsit2DTPCPsiCent[runNumBin]    -> Fill(fCentrality, PsiTPC);
      fHist2DTPCPsiPosCent[runNumBin] -> Fill(fCentrality, PsiTPCPos);
      fHist2DTPCPsiNegCent[runNumBin] -> Fill(fCentrality, PsiTPCNeg);

      fProfileZNCTPCCorr[0] -> Fill(fCentrality, TMath::Cos(2*(PsiTPC - PsiZNCGE)));
      fProfileZNATPCCorr[0] -> Fill(fCentrality, TMath::Cos(2*(PsiTPC - PsiZNAGE)));
      fProfileZNCTPCCorr[1] -> Fill(fCentrality, TMath::Cos(2*(PsiTPC - PsiZNCRC)));
      fProfileZNATPCCorr[1] -> Fill(fCentrality, TMath::Cos(2*(PsiTPC - PsiZNARC)));
      fProfileZNCTPCCorr[2] -> Fill(fCentrality, TMath::Cos(2*(PsiTPC - PsiZNCSF)));
      fProfileZNATPCCorr[2] -> Fill(fCentrality, TMath::Cos(2*(PsiTPC - PsiZNASF)));

      fProfileZNCTPCNegCorr[0] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCNeg - PsiZNCGE))); 
      fProfileZNATPCNegCorr[0] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCNeg - PsiZNAGE)));
      fProfileZNCTPCNegCorr[1] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCNeg - PsiZNCRC))); 
      fProfileZNATPCNegCorr[1] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCNeg - PsiZNARC)));
      fProfileZNCTPCNegCorr[2] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCNeg - PsiZNCSF))); 
      fProfileZNATPCNegCorr[2] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCNeg - PsiZNASF)));

      fProfileZNCTPCPosCorr[0] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCPos - PsiZNAGE)));
      fProfileZNATPCPosCorr[0] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCPos - PsiZNAGE)));
      fProfileZNCTPCPosCorr[1] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCPos - PsiZNAGE)));
      fProfileZNATPCPosCorr[1] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCPos - PsiZNAGE)));
      fProfileZNCTPCPosCorr[2] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCPos - PsiZNAGE)));
      fProfileZNATPCPosCorr[2] -> Fill(fCentrality, TMath::Cos(2*(PsiTPCPos - PsiZNAGE)));

      fProfileTPCPosNegCorr -> Fill(fCentrality, TMath::Cos(2*(PsiTPCNeg - PsiZNAGE)));
    }
